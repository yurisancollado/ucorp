...
class Educacion extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('institucion, inicio, titulo, persona_id', 'required'),
			array('persona_id', 'numerical', 'integerOnly'=>true),
			array('institucion', 'length', 'max'=>450),
			array('titulo', 'length', 'max'=>100),
			array('finalizacion', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, institucion, inicio, finalizacion, titulo, persona_id', 'safe', 'on'=>'search'),
		);
	}
