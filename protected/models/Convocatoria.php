...
class Convocatoria extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('inicio, cargo_id', 'required'),
			array('cargo_id', 'numerical', 'integerOnly'=>true),
			array('observaciones', 'length', 'max'=>450),
			array('fin', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, inicio, fin, observaciones, cargo_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'postulacions' => array(self::HAS_MANY, 'Postulacion', 'convocatoria_id'),
		);
	}
